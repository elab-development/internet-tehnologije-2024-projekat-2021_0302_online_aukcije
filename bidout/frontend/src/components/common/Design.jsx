...
const Body = ({ children, className }) => {
  return (
    <p className={`${className} text-base font-normal text-[rgb(107,113,119)]`}>
      {children}
    </p>
  );
};
...
const PrimaryButton = ({ children, className, disabled = false }) => {
  return (
    <button
      type='submit'
      disabled={disabled}
      className={`${className} text-white bg-green font-medium rounded-full text-lg px-16 py-3 hover:bg-primary`}
    >
      {children}
    </button>
  );
...
export const commonClassNameOfInput =
  'w-full p-4 text-sm text-gray-900 border border-gray-200 focus:ring-green focus:border-green';
